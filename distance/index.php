...
if (!empty($_POST)) {
    ...
    if (false === $answer) {
        ...
        curl_setopt($ch, CURLOPT_URL, 'https://www.distance24.org/route.json?stops='.$town1.'|'.$town2);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        
        $answer = curl_exec($ch); // siuntimas ir laukimas atsakymo
        
        $answer = json_decode($answer);
        $DATA->set($answer); // Catchinam naujus duomenis 
    ...
    header('Location: http://localhost/HomeWork/distance/');
    die;
...
<h1> Atstumų API </h1>
    ...
    <?php if(isset($dist)): ?>
    <h2> Budas yra: <?= $method ?></h2>
    <h2> Atstumas yra: <?= $dist ?> KM </h2>
    <img style="width: 50%; float: left;" src="<?= $img1 ?? '' ?>" alt="img1">
    <img style="width: 50%; float: left;" src="<?= $img2 ?? '' ?>" alt="img2">
    <?php endif ?>
